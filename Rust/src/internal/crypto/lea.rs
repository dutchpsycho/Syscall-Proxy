...
#[test]
fn run_encryptor() {
    let out_path = Path::new("src/internal/crypto/encrypted.rs");
    let mut file = File::create(out_path).expect("failed to create encrypted.rs");

    let k = key(); // this is fine — cargo test is one-shot
    writeln!(file, "// AUTOGENERATED — DO NOT EDIT").unwrap();
    writeln!(file, "pub const ENCRYPTION_KEY: [u8; 16] = {:?};", k).unwrap();
    writeln!(file).unwrap();

    for input in INPUTS {
        let encrypted = lea_encrypt_str(input, &k);
        ...
        writeln!(
            file,
            "pub const ENCRYPTED_{}: &[u8] = {}; // \"{}\"",
            const_name,
            format_byte_array(&encrypted),
            input
        )
        .unwrap();
    }
